...
class upload
{
  ...
  /**
     ...
     * 图片上传
     */
    function borrow_qr_zl($data=array()){
    	global $mysql;
    	$user_id = intval($data['user_id']);
    	$borrow_id = intval($data['borrow_id']);
    	$type_id = intval($data['type_id']);
    	$upload_type = intval($data['upload_type']);
    	if ($data['is_borrow']!=-1){
    		if($borrow_id<1){
    			return false;
    		}
    	}
    	if($user_id<1 || $type_id<1 || $upload_type<1){
    		return false;
    	}
    	$file = $data['file'];
    	$key = $data['key'];
    	$filename = $_FILES[$file]['name'][$key];
    	$hzarr = array(".jpg",".gif",".png",".jpeg",".bmp");
    	$ym = date("Y-m",time());
    	$d = date("d",time());
    	$path = ROOT_PATH.'data/upfiles/images/'.$ym.'/'.$d;
    	mkdir($path,0777,true);
    	$now = time();
    	$now .= mt_rand(10000, 99999);
    	$hz = strrchr($filename,'.');
    	if(!in_array($hz,$hzarr)){
    		echo "<script>alert('资料上传失败，{$filename}不是图片格式!');history.go(-1);</script>";
    		exit();
    	}
    	move_uploaded_file($_FILES[$file]['tmp_name'][$key], $path.'/'.$now.$hz);
    	$data_1['litpic'] = 'data/upfiles/images/'.$ym.'/'.$d.'/'.$now.$hz;
    	$re = $this->is_image($data_1['litpic']);
    	if(!$re){
    		@unlink($data_1['litpic']);
    		echo "<script>alert('资料上传失败，{$filename}不是图片或已被破坏!');history.go(-1);</script>";
    		exit();
    	}
    	$data_1['user_id'] = $user_id;
    	$data_1['borrow_id'] = $borrow_id;
    	$data_1['upload_type'] = $upload_type;
    	$data_1['type_id'] = $type_id;
    	if(isset($data['name'])){
    		$data_1['name'] = mysql_real_escape_string($data['name']);
    	}
    	$re = $mysql->db_add("attestation", $data_1);
    	return $re;
    }
}
?>